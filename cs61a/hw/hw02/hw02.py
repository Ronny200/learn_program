...
def interleaved_sum(n, f_odd, f_even):
    ...
    def f(k, is_odd=True):
        if k > n:
            return 0
        if is_odd:
            return f_odd(k) + f(k + 1, is_odd=False)
        else:
            return f_even(k) + f(k + 1, is_odd=True)
...
def count_dollars(total):
    ...
    def count(total, doller):
        if total == 0:
            return 1
        if total < 0 or doller == None:
            return 0
        without_doller = count(total, next_smaller_dollar(doller))
        with_doller = count(total - doller, doller)

        return without_doller + with_doller
...
def shuffle(s):
    ...
    # return [x for pair in zip(s[0:n], s[n:]) for x in pair]
    return [x for i in range(n) for x in (s[i], s[n + i])]

...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current.entrance is not None:
            if current.is_hive:
                return None
            elif current.bees and self.lower_bound <= step <= self.upper_bound:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            self.slow_dict[target] = 5

            def slowed_action(gamestate):
                if gamestate.time % 2 == 0 or self.slow_dict[target] == 0:
                    Bee.action(target, gamestate)
                if self.slow_dict[target] > 0:
                    self.slow_dict[target] -= 1
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.scared_turns > 0:
                if hasattr(self, "slow") and self.slow > 0 and gamestate.time % 2 != 0:
                    pass
                else:

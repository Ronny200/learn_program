...
def pick(paragraphs, select, k):
    ...
    conform = [x for x in paragraphs if select(x)]
    return conform[k] if k < len(conform) else ""
    # END PROBLEM 1
...
def about(keywords):
    ...
    def conform(str):
        str = remove_punctuation(str).lower().split()
        for key in keywords:
            if key in str:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff < text[0] and diff <= limit:
            text[0] = diff
            text[1] = word

    return text[1] if text[0] != 100 else typed_word
...
def furry_fixes(typed, source, limit):
    ...
    # n = min(type_len, source_len)
    # count = 0
    # for i in range(n):
    #     if typed[i] != source[i]:
    #         count += 1
    # error = count + abs(type_len - source_len)
    # return error
    def f(t, s, count=0):
        if count > limit:
            return count
        ...
        if t[0] != s[0]:
            count += 1
        return f(t[1:], s[1:], count)

    return f(typed, source)
...
def report_progress(typed, source, user_id, upload):
    ...
    user = {"id": user_id, "progress": progress}
    upload(user)
...
def time_per_word(words, timestamps_per_player):
    ...
    #     times.append([])
    #     for k in range(len(tpp[i]) - 1):
    #         times[i].append(tpp[i][k + 1] - tpp[i][k])
    times = [
        [tpp[i][k + 1] - tpp[i][k] for k in range(len(tpp[i]) - 1)]
        for i in range(len(tpp))
    ]
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        ...
        for k in player_indices:
            player_time = get_time(times, k, i)
            if player_time < less[1]:
                less[0] = k
                less[1] = player_time

        fast[less[0]].append(words[i])
